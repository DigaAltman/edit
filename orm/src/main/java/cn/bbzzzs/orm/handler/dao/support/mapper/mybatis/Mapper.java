package cn.bbzzzs.orm.handler.dao.support.mapper.mybatis;

import cn.bbzzzs.common.util.StringUtils;
import cn.bbzzzs.orm.common.KeyEnum;
...
public class Mapper extends AbstractMapper {
    ...
    private String generateMapperCode(TableDetail tableDetail, List<TableDetailVo> tableDetailVoList) {
        ...
        tableDetailVoList.stream().filter(td -> !StringUtils.isEmpty(td.getKey()) && KeyEnum.valueOf(td.getKey()) == KeyEnum.UNI).forEach(unionKeyDetail -> {
            ...
            appText
                    .build("    /**\n")
                    .build("     * @param ", unionKeyDetail.getParamName(), " 根据", comment, "查询数据, 推荐使用此方法查询数据. 因为它是基于唯一索引查询的\n")
            ...
            appText
                    .build("    /**\n")
                    .build("     * @param ", unionKeyDetail.getParamName(), " 根据", comment, "判断数据是否已经存在, 推荐使用此方法来确定数据唯一性. 因为他是基于唯一索引查询的\n")
     ...
     * @param isResultType   是否是 resultType
     * @param selectSQL      查询语句
     * @return 返回 <select>...</select>
     */
    private String generateSelect(String annotationText, String id, String resultMap, String parameterType, boolean isResultType, String selectSQL) {
        StringUtils.SBuilder sb = new StringUtils.SBuilder();
        if (!StringUtils.isEmpty(annotationText)) {
            sb.build("    <!--", annotationText, "-->\n");
        }
        ...
        if (parameterType != null) {
            sb.build("parameterType=\"", parameterType, "\"");
        }
    ...
    private String generateMapperXml(TableDetail tableDetail, List<TableDetailVo> tableDetailVoList) {
        ...
        // 生成 <select>SELECT * FROM table</select>
        sb.build(generateSelect("查询所有数据", "selectAll", ID, null, false, "SELECT <include refid=\"baseColumn\" FROM " + tableDetail.getName()));
        ...
        if (super.ID_INDEX.size() > 0) {
            ...
            sb.build(generateSelect("根据主键查询数据, 推荐使用这个方法查询数据", "selectById", ID, parameterType, false, selectById.toString()));
        ...
        super.ONLY_INDEX.forEach(detailVo -> {
            ...
            StringUtils.SBuilder selectSql = StringUtils.builder("SELECT <include refid = \"baseColumn\"/> FROM ", tableDetail.getName(), " WHERE ", detailVo.getFieldName(), " = #{", detailVo.getParamName() + "}");
            sb.build(generateSelect(annotationBuilder.toString(), selectId, ID, fieldParameterType, false, selectSql.toString()));


            // 基于唯一索引判断数据是否存在
            String containId = "containBy" + detailVo.getQueryParamName();
            sb.build(generateSelect("根据" + comment + "判断数据是否已经存在, 推荐使用此方法来确定数据唯一性. 因为他是基于唯一索引查询的", containId, "int", fieldParameterType, false, StringUtils.builder("SELECT COUNT(1) FROM ", tableDetail.getName(), " WHERE ", detailVo.getFieldName(), " = #{", detailVo.getParamName(), "}").toString()));
        });
        ...
        super.MNL_INDEX.forEach((keyName, keyList) -> {
            ...
            sb.build(generateSelect(annotationText.append("查询数据, 此方法虽然比不上主键和唯一索引, 但他也是走索引查询的").toString(), mappedStatementID.toString(), ID, parameterType, false, StringUtils.builder("SELECT <include refid = \"baseColumn\"/> FROM ", tableDetail.getName(), " WHERE ", expr.toString()).toString()));
        });
